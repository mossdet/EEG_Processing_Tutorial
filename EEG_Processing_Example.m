...
%% Set flag to enable/disbale plotting HFO vents
plot_ok = 1;

%% Loop through EEG files
for fi = 1:length(eeg_files)
    %% Get EEG Filepath and patient name
    eegFilepath = strcat(eegs_dir, eeg_files{fi});
    eegFilepath = eegFilepath{1};
    [pathstr, pat_name, ext] = fileparts(eegFilepath);

    %% Get filepath from HFO detections
    ...
    disp(strcat("Processing file ", num2str(fi),'/', num2str(length(eeg_files)), ": ", pat_name));
    ...
    for chidx = 1:length(mtg_labels)
        ...
        disp(strcat("Channel ", num2str(chidx),'/', num2str(length(mtg_labels)), ": ", eeg_ch));
        ...
        for hfo_idx = 1:nr_ch_hfo
            ...
            duration_ms = (hfo_time(end)-hfo_time(1))*1000;
